...
from django_business_rules.tests.dummy_business_rule import DummyBusinessRule, \
    DummyBusinessRuleWithName, DummyBusinessRuleWithVariables
...
class BusinessRuleTests(TestCase):
    ...
    def test_should_update_rule_model_in_db(self):
        ...
        self.assertFalse(BusinessRuleModel.objects.filter(name=expected_name).exists())
        ProductBusinessRule.generate()
        self.assertEqual(BusinessRuleModel.objects.get(name=expected_name).description, '')
        # WHEN
